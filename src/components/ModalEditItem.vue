...
const { updateItemDB } = useInventory()
...
// Salin data barang yang diklik ke dalam form sementara
const form = ref({ ...props.item })

const saveChanges = async () => {
  // Timpa data di layar agar langsung berubah
  Object.assign(props.item, form.value)
  // Simpan permanen ke database Supabase
  await updateItemDB(props.item)
  emit('close')
}
</script>
    ...
    <div class="bg-slate-900 border border-slate-800 rounded-xl max-w-md w-full p-6 shadow-2xl">
      ...
      <form @submit.prevent="saveChanges" class="space-y-4">
        ...
        <div class="flex items-center gap-2 pt-2">
          <input v-model="form.auto_buffer" type="checkbox" id="editBuf" class="rounded bg-slate-950 border-slate-700 text-sky-500 w-4 h-4 cursor-pointer" />
          <label for="editBuf" class="text-xs text-slate-300 cursor-pointer">Aktifkan Auto-Buffer +30%</label>
        </div>
